...
import { WalletTransfer, KingMakerzTransfer } from "../../utils/Constant";
...
import { LotteryPropType, LotteryForminitialValues } from "../../Type/Forms/Lottery/Lottery";
...
const LotteryTransfer = () => {
  useEffect(() => {
    GetWalletBalance("CommissionWallet");
    setUsername(localStorage.getItem("MemberName") as string);
  }, []);
  const { getWalletBalance, doKingMakerzTransfer, loading } = useTransferFundService();
  ...
  const handleTransfer = async (values: LotteryPropType) => {
    ...
    if (confirmed) {
      ...
      const res = await doKingMakerzTransfer(param);
      if (res[0].StatusCode == "1") {
  ...
  return (
    <>
      <Breadcrumbs mainTitle={KingMakerzTransfer} parent={WalletTransfer} ChildName={KingMakerzTransfer} />
      <Container fluid>
        {loading && <Loader />}
        <Row>
          <Col xl="4">
            ...
            <Col xl="12" style={{ display: 'flex' }}>
              <div><i className="fa fa-info-circle"></i></div><div style={{ paddingLeft: '10px' }}>
                Minimum Transfer should be $100 <br /> N0 Transaction Fee Applicable
              </div>
            </Col>
          </Col>
          <Col xl="8">
            <Card>
              <CardBody>
                <div className="gap-3 pills-blogger">
                  <Formik
                    initialValues={LotteryForminitialValues}
...
export default LotteryTransfer;
